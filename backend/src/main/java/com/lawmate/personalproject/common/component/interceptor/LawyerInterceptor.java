...
@Slf4j
public class LawyerInterceptor implements HandlerInterceptor{
...
//        log.info("4- 인터셉터 최종 여부 {}", true);
            ...
            return Stream.of(request)
                    ...
                    .peek(token -> log.info("1- lawyerId 인터셉터 토큰 로그 Bearer 포함 : {}", token))
                    .map(token -> jwt.getPayload(token).get("lawyerId", Long.class))
                    .map(id -> repository.findById(id))
                    .filter(id -> id.isPresent())
                    .peek(id -> log.info("2- 인터셉터 lawyer id : {}", id))
                    .findFirst()
                    .isPresent();
        }   // request
